...
Directory Resolution (in order of precedence):
    ...
    2. ``PIPELINE_STATE_DIR`` environment variable (identities/ subdirectory)
...
def _identity_dir(state_dir: Optional[str] = None) -> str:
    ...
    2. ``PIPELINE_STATE_DIR`` env var + /identities/ subdirectory
    ...
    env_dir = os.environ.get("PIPELINE_STATE_DIR")
    if env_dir:
